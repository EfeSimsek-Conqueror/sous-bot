...
| Auth | ~~mock buttons~~ **SUPERSEDED (session 8):** web has **real Google OAuth (PKCE)** via `SignInSheet.tsx`; Apple is disabled "Coming soon". Android's Google sign-in is wired but still failing — see current state. |
...
## ▶ CURRENT STATE — resume here (end of session 9, 2026-07-25)
- **Web: DONE & live** at `https://sous-bot.vercel.app` (Vercel, from collab repo `EfeSimsek-Conqueror/sous-bot`, deploys on push to its `main`, root dir `apps/web`). Backend = `food` project. **Google sign-in works on web** (branded `SignInSheet.tsx` from "Get started"; Apple "Coming soon"). Vercel env vars point at `food` (URL/anon/functions).
- **Landing page REPLACED AGAIN (session 9) with the user's new Claude Design** — project `28f8c00b-5baf-43de-a51c-4fda160f4599`, file `Sousbot Landing.dc.html`, read via the `claude_design` MCP (`DesignSync`). It's a **scene-locked WebGL stage**, not a scrolling page: 5 scenes (Welcome / The loop / Recipes / Pricing / Start cooking), wheel+arrows+swipe+dots to step, copy and cards flying in from opposite sides on a rotateY arc, and a **real three.js terracotta pot** behind them whose lid lifts and swings off, steam thickens, ingredients rise and orbit, and pot sinks out of frame by the last scene. Files: `apps/web/components/LandingPage.tsx` (markup + nav) and `apps/web/components/landing/potScene.ts` (the 3D scene). `three` + `@types/three` are now **deps of apps/web** (dynamic `import("three")` → own chunk, no CDN). `apps/web/public/brand/logo-{mark,full-dark}.svg` added (real files, not symlinks — the design's uploads/ are byte-identical to `brand/`). **Built clean and verified with a CDP screenshot harness at 1920/1440/1280/390 — all 5 scenes, sign-in sheet opens from a CTA, reduced-motion path checked. DEPLOYED & LIVE** at `https://sous-bot.vercel.app` (collab-repo commit `bc1ae05`, 2026-07-25; Vercel Production status success, pot renders live, console clean).
...
| 5 | Brand assets (logo/icon/favicon) | ✅ done (USER pot glyph) | v1=egg,v2=mushroom/phallic,v3=fork ALL rejected. **FINAL = user-provided steaming-pot glyph**, used verbatim (source stashed `brand/_source-pot-icon-set.svg`). All assets regenerated `8dea919`; coordinator Read+approved icons + both lockups (fixed lockup viewBox 520→600 so "Sousbot" wasn't clipped). Palette: tile `#D68D50`, glyph white, dark tile `#2B2018`, on-light `#D68D50`. ~~TODO: re-copy icons into the apps~~ **DONE in session 2 (`25dbfa8`)** — android mipmaps replaced + adaptive fg/bg rasterized, web icons are symlinks into `brand/` (dereferenced to real files for deploy). Session 9 also added real-file copies of `logo-mark.svg` + `logo-full-dark.svg` to `apps/web/public/brand/` for the landing. |
...
| 9 | Deploy web → sousbot.ai | ✅ LIVE on **Vercel** / ⏳ custom domain | **Live: https://sous-bot.vercel.app** — Vercel, sourced from the collab repo **`EfeSimsek-Conqueror/sous-bot` @ `main`, root dir `apps/web`**, auto-deploys on push. Backend = `food` (`retjqmizcapeplxfznhg`); `NEXT_PUBLIC_*` env vars set there; `CORS_EXTRA_ORIGINS=https://sous-bot.vercel.app` set on Supabase. Deploy + verification procedure and its gotchas → the SESSION 9 block below. **⚠ RAILWAY IS LEGACY — ignore it.** The old path (Railway service `sousbot-web`, project `e18e1f11-…`, mirror repo `Koragan/sousbot-web`, URL `sousbot-web-production.up.railway.app`) is **superseded and no longer the deploy target**; it's kept only in the historical session blocks. **Still open: register `sousbot.ai`**, then attach it in Vercel and reconcile `supabase/functions/_shared/cors.ts` (it hardcodes `sousbot.app` — `.app`, not `.ai`). |
| 11 | Landing page v3 (imported Claude Design) | ✅ done + deployed | See the "Landing page REPLACED AGAIN" bullet in CURRENT STATE and the SESSION 9 block. Scene-locked 5-scene WebGL stage; `components/LandingPage.tsx` + `components/landing/potScene.ts`; `three` bundled. Verified at 1920/1440/1280/390 + live. Supersedes the session-7 11-element landing entirely. |
...
**▶▶ SESSION 9 (landing v3) — 2026-07-25 — DONE, SHIPPED.** The user's external design landed, was ported, and is **live at https://sous-bot.vercel.app**. Read the "Landing page REPLACED AGAIN" bullet above for what it is. **Port decisions worth knowing before you touch it:**
- Idiomatic React, **no `dangerouslySetInnerHTML` for markup** this time (the previous landing did that). The DC runtime's `{{ }}` / `style-hover` / `<sc-for>` map to state, CSS `:hover` classes (`.sbl-*` in the `CSS` const) and `.map()`. Confirmed against the design project's `support.js`: `sc-for` is a plain list map, `hint-placeholder-count` is editor-only, and `style-hover` isn't in the runtime at all — CSS `:hover` is the right equivalent.
- The page **locks document scroll** (`html/body overflow:hidden`, restored on unmount) — it owns the viewport.
- **The design is desktop-only; the mobile layout is ours.** Under 900px the two columns stack into a centred column (the per-scene widths are inline, so the media query needs `!important` to out-rank them), the scrim flips from left/right to top/bottom, and there's a fit-scale that shrinks a stacked scene on short viewports.
- **`distFactor` / `lookY` in `potScene.ts` are ours, not the design's.** The design's camera distances frame perfectly at ~1920, but the pot's on-screen size tracks viewport *height* while the copy columns track *width* — so at 1280–1600 the pot crept under the hero paragraph. `distFactor` pulls the camera back 14–32% there, and 2.4× + `lookY 1.78` in portrait so the pot settles low behind the glass cards instead of peeking through a gap. `potY`/`lidY` exits are multiplied by `distFactor` too, or the lid hovers in shot on mobile instead of leaving frame.
- `prefers-reduced-motion`: fly-ins become a plain crossfade, every ambient oscillation is zeroed (`MO`), steam is hidden, the camera snaps instead of easing, and the render loop goes dirty-only.
- The old 11-element landing is **gone** (this replaced it). `components/Reveal.tsx` and `components/WelcomeScreen.tsx` are now unused leftovers — safe to delete whenever.
- **DEPLOYED.** Flow used (repeat this): clone `EfeSimsek-Conqueror/sous-bot` (it's the full monorepo, scrubbed, with its own ongoing history — *not* a single fresh commit any more), `diff -rq` its `apps/web` against the monorepo's to confirm no drift from Efe, copy only the changed files, secret-scan, `npm ci && npm run build` in the clone to prove Vercel will succeed, then commit + push to `main`. **Keep `apps/web/tokens.source.json` — it exists only in the collab repo** (the prebuild's fallback when `_design/tokens.json` is outside the Vercel build context) and it's currently in sync with `_design/tokens.json`.
- **Deploy-verification gotchas (cost ~20 min this session):** (a) **don't poll the live URL in a tight loop** — ~20 curls in 6 min tripped Vercel's bot mitigation (`x-vercel-mitigated: challenge`, HTTP 403 "Security Checkpoint") from this IP for ~15 min, which looks exactly like a failed deploy but isn't; (b) **grepping the root HTML for landing markers proves nothing** — the landing is client-rendered by `AppShell` after the session check, so the SSR HTML never contains `sbl-*`, and the component sits in a lazily-loaded chunk that isn't referenced in the initial HTML either; (c) chunk filenames are content-hashed and Vercel's hashes ≠ a local build's, so you can't verify by fetching a locally-derived chunk path; (d) headless Chrome gets "Failed to verify your browser (Code 29)" from the checkpoint unless you add **`--disable-blink-features=AutomationControlled`** — with that flag the CDP harness renders the live site fine. **The reliable non-browser signal is GitHub:** `gh api repos/EfeSimsek-Conqueror/sous-bot/deployments` (sha + environment) and `.../commits/<sha>/status` ("Deployment has completed" / success).
- **Still open:** re-check the landing on a real phone (the stacked layout is only headless-verified), and register `sousbot.ai`.

---
### ⚠ Everything below this line is HISTORICAL — read for context, don't follow its instructions
These blocks are kept for the reasoning behind past decisions. **Their deploy instructions are all superseded:**
they talk about Railway and the `Koragan/sousbot-web` mirror, which are dead. The live path is Vercel ←
`EfeSimsek-Conqueror/sous-bot` (status row #9 + the SESSION 9 block). They also describe the session-7
11-element landing, which no longer exists — session 9 replaced it wholesale.
...
## Log
- **2026-07-25 — SESSION 9.** Imported the user's new Claude Design landing (project `28f8c00b…`) via the
  `claude_design` MCP and ported it to `apps/web`: scene-locked 5-scene WebGL stage, real three.js pot
  (`components/landing/potScene.ts`), `three` bundled as a dep. Added the mobile stacked layout and the
  `distFactor`/`lookY` camera adjustments the source doesn't have. Build clean; verified via a CDP screenshot
  harness at 1920/1440/1280/390 plus sign-in-sheet and reduced-motion checks. **Deployed** to the collab repo
  (`bc1ae05`) → Vercel Production, verified live (pot renders, console clean).
